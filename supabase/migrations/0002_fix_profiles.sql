-- =====================================================================
-- Studio — Fix migration: adapt the Supabase-starter `profiles` table
-- to our schema (adds missing columns, backfills, then re-runs the
-- rest of the schema cleanly).
--
-- Safe to run multiple times. Run this ONCE in the SQL Editor, then
-- 0001_init.sql will succeed if you ever need to re-run it.
-- =====================================================================
...
-- 2. Profiles — add missing columns (idempotent) -----------------------
alter table public.profiles add column if not exists email text;
alter table public.profiles add column if not exists phone text;
alter table public.profiles add column if not exists role user_role not null default 'client';
alter table public.profiles add column if not exists whatsapp_number text;
alter table public.profiles add column if not exists created_at timestamptz not null default now();

-- 3. Backfill email from auth.users for any existing rows --------------
update public.profiles p
set email = u.email
from auth.users u
where p.id = u.id and (p.email is null or p.email = '');

-- 4. Now make email NOT NULL (only if all rows are filled) -------------
do $$ begin
  alter table public.profiles alter column email set not null;
exception when others then
  raise notice 'Could not set email NOT NULL — some rows missing email. Sign in again to fix.';
end $$;

-- 5. Backfill full_name where missing -----------------------------------
update public.profiles p
set full_name = coalesce(p.full_name, split_part(u.email, '@', 1))
from auth.users u
...
-- Allow inserts for any starter-policies that may have been auto-created
...
-- 13. Realtime publication (idempotent) ---------------------------------
do $$ begin
